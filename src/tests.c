#include <stdlib.h>

#include "lib.h"

int runTests() {
    // type tests
    expect("U64 size", sizeof(U64), 8);
    expect("U8 range", (U8)(-1), 255);
    expect("size_t is 64 bit", sizeof(size_t), sizeof(U64));

    // mem tests
    ArenaAllocator arena;
    arenaInit(&arena, 1);
    expect("arena page size", arena.size, 4096);
    arenaFree(&arena);

    arenaInit(&arena, 0);
    expect("default arena size", arena.size, GB(1));
    U8* data = arenaAllocZero(&arena, 12);
    expect("arena commit size", arena.committed, KB(8));
    expect("arena allocate size", arena.allocated, 16);
    expect("arena zero data", data[11], 0);
    data[11] = 14;    
    expect("arena data", data[11], 14);
    arenaFree(&arena);

    // SHA-1 tests
    U8 hash[20];
    char hex[41];
    hex[40] = '\0';
    
    sha1(hash, (U8*)"", 0);
    bytesToHex(hex, hash, 20);
    expectCharPtr("SHA 1 (blank)", hex, "da39a3ee5e6b4b0d3255bfef95601890afd80709");

    sha1(hash, (U8*)"a", 1);
    bytesToHex(hex, hash, 20);
    expectCharPtr("SHA 1 (a)", hex, "86f7e437faa5a7fce15d1ddcb9eaeaea377667b8");

    sha1(hash, (U8*)"The quick brown fox jumps over the lazy dog", 43);
    bytesToHex(hex, hash, 20);
    expectCharPtr("SHA 1 (dog)", hex, "2fd4e1c67a2d28fced849ee1bb76e7391b93eb12");

    sha1(hash, (U8*)"The quick brown fox jumps over the lazy cog", 43);
    bytesToHex(hex, hash, 20);
    expectCharPtr("SHA 1 (cog)", hex, "de9f2c7fd25e1b3afad3e85a0bd17d9b100db4b3");

    // report
    if (expect_failed) {
        PRINT(CC_BRIGHT_RED, "Some tests failed!\n");
        return EXIT_FAILURE;
    } else {
        PRINT(CC_GREEN, "All tests passed!\n");
        return EXIT_SUCCESS;
    }
}